...
function rolesForActor(actorRole: UserRole): UserRole[] {
  ...
  if (actorRole === UserRole.HR) {
    return [UserRole.EMPLOYEE, UserRole.DEPARTMENT_HEAD, UserRole.HR];
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ...
              : "Create a new employee account for Knowledge Hub."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Department {isEdit ? "" : "(required for Employee, Student, Manager)"}</Label>
            <Select
              value={departmentId || "none"}
              onValueChange={(v) =>
                setValue("departmentId", v === "none" ? null : v)
              }
            >
              <SelectTrigger>
