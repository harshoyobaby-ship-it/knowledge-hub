...
export default function HREmployeeDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Overall training progress</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            · {insights.timeSpentMinutes} minutes spent learning
          </p>
        </CardContent>
      </Card>
      ...
      {insights.quizScores?.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">Quiz scores</CardTitle></CardHeader>
          <CardContent className="divide-y">
            {insights.quizScores.map((q: { quizId: string; quizTitle: string; score: number; passed: boolean }) => (
              <div key={q.quizId} className="flex justify-between py-3 text-sm">
                <span>{q.quizTitle}</span>
                <span className={q.passed ? "text-emerald-600" : "text-amber-600"}>
